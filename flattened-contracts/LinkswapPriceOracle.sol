...
// File: contracts/LinkswapPriceOracle.sol
...
contract LinkswapPriceOracle is ILinkswapPriceOracle {
    ...
    constructor(
        ...
        IUniswapV2Pair _pair =
            IUniswapV2Pair(
                UniswapV2Library.pairFor(_factory, _wethToken, _yflToken)
            );
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
        ...
        returns (uint256)
    {
        if (token == yflToken) {
            ...
            require(
                ...
                "LinkswapPriceOracle: INVALID_USD_PRICE"
            );
            // multiply by 10^18 because WETH has 18 dp
            uint256 wethAmount = usdAmount.mul(10**18) / uint256(usdPerWeth);
            return consult(wethToken, wethAmount);
            ...
            int256 usdPrice;
            if (token == linkToken) {
                usdPrice = IChainlinkOracle(linkUsdChainlinkOracle)
                    .latestAnswer();
            } else if (token == wethToken) {
