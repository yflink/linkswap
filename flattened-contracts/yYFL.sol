...
// File: contracts/interfaces/IYFLPurchaser.sol
...
// File: contracts/interfaces/IyYFL.sol
...
contract yYFL is IyYFL, ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            _yfl != address(0) && _treasury != address(0),
            "yYFL: ZERO_ADDRESS"
        );
        _setupDecimals(ERC20(_yfl).decimals());
        YFL = IERC20(_yfl);
    ...
    function stake(uint256 amount) external override nonReentrant {
        ...
        uint256 shares =
            totalSupply() == 0
                ? amount
                : (amount.mul(totalSupply())).div(YFL.balanceOf(address(this)));
        YFL.safeTransferFrom(msg.sender, address(this), amount);
        _mint(msg.sender, shares);
    ...
    function convertTokensToYfl(
        ...
        IYFLPurchaser(yflPurchaser).purchaseYfl(tokens);
        require(
            YFL.balanceOf(address(this)) > yflBalanceBefore,
            "yYFL: NO_YFL_PURCHASED"
        );
    }
    ...
    function withdraw(uint256 shares) external override nonReentrant {
        ...
        _checkVoteExpiry();
        require(
            shares <= balanceOf(msg.sender).sub(voteLockAmount[msg.sender]),
            "yYFL: INSUFFICIENT_BALANCE"
        );
        uint256 yflAmount =
            (YFL.balanceOf(address(this))).mul(shares).div(totalSupply());
        _burn(msg.sender, shares);
        if (block.number < earlyWithdrawalFeeExpiry[msg.sender]) {
            uint256 feeAmount =
                yflAmount.mul(earlyWithdrawalFeePercent) / 1000000;
            YFL.safeTransfer(
                treasury,
    ...
    function getStakeYflValue(address staker)
        external
        view
        override
        returns (uint256)
    {
        return
            (YFL.balanceOf(address(this)).mul(balanceOf(staker))).div(
                totalSupply()
            );
    }
    ...
    function propose(
        ...
        require(targets.length <= MAX_OPERATIONS, "yYFL: TOO_MANY_ACTIONS");
        require(
            (YFL.balanceOf(address(this)).mul(balanceOf(msg.sender))).div(
                totalSupply()
            ) >= minYflForProposal,
            "yYFL: INSUFFICIENT_YFL_FOR_PROPOSAL"
        );
        ...
        proposals[id] = Proposal({
            ...
            totalForVotes: 0,
            totalAgainstVotes: 0,
            quorumVotes: YFL.balanceOf(address(this)).mul(quorumPercent) /
                1000000,
            executed: false
        });
        hasActiveProposal[msg.sender] = true;
        ...
        nonReentrant
    {
        ...
        require(!proposal.executed, "yYFL: PROPOSAL_ALREADY_EXECUTED");
        {
            ...
            require(
                ...
                "yYFL: INVALID_PROPOSAL_ID"
            );
            require(
                block.number >= proposal.endBlock,
                "yYFL: PROPOSAL_IN_VOTING"
            );
            hasActiveProposal[proposal.proposer] = false;
            uint256 totalVotes =
    ...
    function setMinYflForProposal(uint256 _minYflForProposal)
        external
        override
        onlyThis
    {
        // min 0.01 YFL, max 520 YFL (1% of total supply)
        require(
            _minYflForProposal >= 1e16 && _minYflForProposal <= 520 * (1e18)
        );
        minYflForProposal = _minYflForProposal;
    }
