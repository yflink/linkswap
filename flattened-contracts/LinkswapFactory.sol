...
// File: contracts/interfaces/ILinkswapPriceOracle.sol
...
// File: contracts/interfaces/ILinkswapCallee.sol
...
// File: contracts/LinkswapPair.sol
...
// File: contracts/LinkswapFactory.sol
...
contract LinkswapPair is ILinkswapPair, ReentrancyGuard {
    ...
    modifier onlyGovernance() {
        require(
            msg.sender == ILinkswapFactory(factory).governance(),
            "Pair: FORBIDDEN"
        );
        _;
    }
        ...
        returns (bool feeOn)
    {
        uint256 protocolFeeFractionInverse =
            ILinkswapFactory(factory).protocolFeeFractionInverse();
        feeOn = protocolFeeFractionInverse != 0;
        uint256 _kLast = kLast; // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) {
                        ILinkswapFactory linkswapFactory =
                            ILinkswapFactory(factory);
                        uint256 treasuryProtocolFeeShare =
                            linkswapFactory.treasuryProtocolFeeShare();
                        _mint(
                            linkswapFactory.treasury(),
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                ILinkswapCallee(to).linkswapCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
                ...
                if (priceAtLastSlippageBlocks == 0) {
                    ...
                    bool resetSlippage =
                        lastSlippageBlocks.add(
                            ILinkswapFactory(factory).maxSlippageBlocks()
                        ) < block.number;
                    uint256 lastPrice =
                        resetSlippage
                            ? lastSwapPrice
